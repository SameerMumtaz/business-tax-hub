    ...
    <div className="space-y-4">
      {mapUrl && (
        <Card className="overflow-hidden">
          <iframe src={mapUrl} className="w-full h-64 border-0" title="Job Sites Map" />
        </Card>
      )}
      ...
      {sites.length === 0 ? (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground">{t("map.noSites")}</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <Card key={site.id}>
                <CardContent className="pt-4 space-y-2">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {siteJobs.length} {siteJobs.length !== 1 ? t("jobs.jobPlural") : t("jobs.job")}: {siteJobs.map((j) => j.title).join(", ")}
                  </div>
                  {site.latitude != null && (
