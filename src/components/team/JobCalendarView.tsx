...
function detectConflicts(
  ...
  if (totalAfterDrop > 12) {
    conflicts.push({ type: "overloaded", message: `Day would have ${totalAfterDrop.toFixed(1)}h of work scheduled` });
  }
            ...
            <div key={dateStr} className={cn("rounded-lg border transition-all flex flex-col", isToday && "ring-2 ring-primary/50", isDragTarget && showConflicts.length > 0 && "ring-2 ring-red-500/60 bg-red-500/5", isDragTarget && showConflicts.length === 0 && "ring-2 ring-primary/60 bg-primary/5", isGapSuggestion && !isDragTarget && "ring-2 ring-emerald-500/40 bg-emerald-500/5", !isDragTarget && !isGapSuggestion && "border-border bg-card", !isCurrentMonth && "opacity-50")} onDragOver={(e) => handleDragOver(e, dateStr)} onDragLeave={handleDragLeave} onDrop={(e) => handleDrop(e, dateStr)}>
              ...
              <div className="flex-1 p-1 overflow-y-auto space-y-0">
                ...
                {dayJobs.map((job, idx) => {
                  ...
                  return <div key={`${job.id}-${dateStr}-${idx}`}><div draggable={canDrag} onDragStart={(e) => handleDragStart(e, job, dateStr)} onDragEnd={handleDragEnd} onClick={() => { if (!wasDragging.current && !isRescheduled) onJobClick?.(job); }} style={{ minHeight: `${blockHeight}px` }} className={cn("group rounded-md border px-2 py-1.5 transition-all select-none flex flex-col", isRescheduled ? "opacity-40 bg-muted/50 border-dashed border-muted-foreground/30 cursor-default line-through decoration-muted-foreground/40 pointer-events-none" : cn("cursor-pointer hover:shadow-sm", STATUS_BG[displayStatus] || STATUS_BG.scheduled, canDrag && "hover:ring-1 hover:ring-primary/30"), isDragging && "opacity-30 scale-95")}><div className="flex gap-1.5 flex-1 pointer-events-none"><div className={cn("w-1 rounded-full shrink-0 self-stretch", STATUS_ACCENT[displayStatus] || STATUS_ACCENT.scheduled)} /><div className="min-w-0 flex-1 flex flex-col">{canDrag && <GripVertical className="h-3 w-3 text-muted-foreground/40 group-hover:text-muted-foreground shrink-0 self-end cursor-grab" />}<div className={cn("text-xs font-semibold truncate", STATUS_TEXT[displayStatus])}>{job.title}</div><div className="flex flex-wrap items-center gap-x-2 gap-y-0 mt-0.5">{job.start_time && <span className="text-[10px] text-muted-foreground flex items-center gap-0.5"><Clock className="h-2.5 w-2.5" />{formatTime12(job.start_time)}</span>}{job.estimated_hours && <span className="text-[10px] text-muted-foreground font-mono">{job.estimated_hours}h</span>}</div>{site && <span className="text-[10px] text-muted-foreground flex items-center gap-0.5 truncate mt-0.5"><MapPin className="h-2.5 w-2.5 shrink-0" /><span className="truncate">{site.name}</span></span>}{assignments.filter((a) => a.job_id === job.id).length > 0 && <div className="flex flex-wrap gap-0.5 mt-1">{assignments.filter((a) => a.job_id === job.id).slice(0, 3).map((a) => <span key={a.id} className="text-[9px] bg-background/80 border border-border/50 rounded px-1 py-0 text-muted-foreground">{a.worker_name.split(" ")[0]}</span>)}{assignments.filter((a) => a.job_id === job.id).length > 3 && <span className="text-[9px] text-muted-foreground">+{assignments.filter((a) => a.job_id === job.id).length - 3}</span>}</div>}{isRescheduled ? <span className="text-[9px] text-destructive/70 font-medium no-underline mt-auto" style={{ textDecoration: "none" }}>↗ Rescheduled</span> : job.job_type === "recurring" ? <span className="text-[9px] text-muted-foreground/60 italic mt-auto">↻ {job.recurring_interval}</span> : null}</div></div></div>{gapHeight > 8 && !dragJob && <div className="mx-1 border-l-2 border-dashed border-muted-foreground/15 flex items-center pl-2" style={{ height: `${Math.min(gapHeight, 80)}px` }}><span className="text-[9px] text-muted-foreground/40 font-mono">{gapMinutes >= 60 ? `${(gapMinutes / 60).toFixed(1)}h free` : `${gapMinutes}m free`}</span></div>}{showDropAfter && <div className={cn("rounded-md border-2 border-dashed flex items-center justify-center cursor-pointer transition-all my-1", dragOverDate === dateStr && dragOverIndex === idx + 1 ? "h-9 border-primary bg-primary/15 shadow-sm" : "h-7 border-primary/30 bg-primary/5 hover:border-primary hover:bg-primary/10")} onDragOver={(e) => handleCardDragOver(e, dateStr, idx + 1)} onDrop={(e) => handleCardDrop(e, dateStr, idx + 1)}><span className={cn("text-[10px] font-medium pointer-events-none", dragOverDate === dateStr && dragOverIndex === idx + 1 ? "text-primary" : "text-primary/60")}>{isLastCard ? "↓ Move here" : "Move here"}</span></div>}</div>;
                })}
