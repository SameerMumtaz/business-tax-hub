                    ...
                    <tr key={e.id} className={selected.has(e.id) ? "bg-primary/5" : ""}>
                      ...
                      <td>
                        {editingCategoryId === e.id ? (
                          ...
                          <button onClick={() => setEditingCategoryId(e.id)} className="group flex items-center gap-1"><Badge variant="secondary" className="text-[11px] font-normal min-w-[100px] max-w-[130px] px-3 py-1 justify-center text-center leading-tight">{e.category}</Badge><Pencil className="h-3 w-3 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" /></button>
                        )}
                      </td>
